...
与图形化界面中的文本编辑器一样，vi 是命令行下，对文本文件进行编辑的绝佳选择。

vim 是 vi 的加强版本，兼容 vi 的所有指令，它不仅能编辑文本，而且还具有 shell 程序编辑的功能，可以以不同颜色的字体来辨别语法的正确性，极大方便了程序的设计和编辑性。

## 二、vi、vim 编辑器三种工作模式

命令模式（Command mode），该模式下，所敲的按键，都被当作命令执行，不能自由进行文本编辑。

输入模式（Insert mode）也称“编辑模式”、“插入模式”。该模式下，可以对文件内容进行自由编辑。
...
vim 兼容全部的 vi 功能，使用 vi / vim 编辑器编辑文件，语法：`vi 文件路径` / `vim 文件路径`；

如果文件路径的文件不存在，那么此命令会用于创建新文件，并编辑.。

如果文件路径的文件存在，那么此命令用于编辑已有文件。
...
在命令模式，执行输入键盘指令，比如输入 `i`，可以进入输入模式。
